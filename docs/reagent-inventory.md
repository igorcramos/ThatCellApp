...
Manual code entry works in every supported browser. Live camera and image decoding use the browser-native `BarcodeDetector` API when it is reliable, with the locally bundled ZXing Browser 0.2.0 decoder as a Safari/iOS and cross-browser fallback. If a native detector is present but fails, the scanner switches to ZXing. Camera access additionally requires HTTPS or `localhost`, browser permission, and `getUserMedia`. When live camera access is unavailable (including restricted in-app browsers), the UI explains the reason and keeps three independent paths available: take a photo with the device camera, choose an existing image, or type the code manually. The scanner supports common QR, EAN, UPC, Code 128, Code 39, and Data Matrix formats.

The camera request first prefers the rear camera and falls back to any available camera if the browser rejects those constraints. A pending permission prompt times out after 30 seconds so the UI can recover; if the delayed request later succeeds, its tracks are immediately stopped. Detailed permission, missing-device, busy-camera, embedded-browser, and security messages guide the user without hiding photo or manual entry. ZXing licensing is recorded in [third-party notices](third-party-notices.md).
