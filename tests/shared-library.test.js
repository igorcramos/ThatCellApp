...
const atomicRunsMigration = fs.readFileSync(path.join(root, "supabase/2026-08-27_atomic_differentiation_runs.sql"), "utf8");
...
assert.match(index, /id="differentiationCellLineSummary"/);
assert.doesNotMatch(index, /id="differentiationCellLineCheckboxes"/);
assert.match(app, /db\.rpc\("save_differentiation_run"/);
assert.match(app, /if \(snapshotIds\.length > 0\) return snapshotIds;/, "saved run lineages must take precedence over the current culture");
assert.match(atomicRunsMigration, /create or replace function public\.save_differentiation_run/);
assert.match(atomicRunsMigration, /security definer/);
assert.match(atomicRunsMigration, /insert into public\.differentiation_run_cell_lines/);
assert.match(atomicRunsMigration, /as permissive for insert to authenticated/);
assert.match(atomicRunsMigration, /from public\.culture_cell_lines link/);
assert.match(atomicRunsMigration, /mapped\.cell_line_id is null/, "a well's direct lineage must take precedence over its culture fallback");
assert.match(atomicRunsMigration, /Map the selected source well\(s\) before starting/);
assert.match(atomicRunsMigration, /^begin;[\s\S]*commit;\s*$/m, "run, wells, and lineage snapshot must be saved atomically");
