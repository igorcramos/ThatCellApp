...
const weeks = ScheduleCalendar.buildPrintableWeeks(month, { maxUnits: 10 });
const days = weeks.flat().filter(cell => cell?.date === '2026-09-15');
assert.equal(days.length, 7, 'crowded days continue in additional dated week rows');
assert.deepEqual(days.flatMap(day => day.entries.map(entry => entry.id)), denseEntries.map(entry => entry.id), 'continuations preserve every entry exactly once');
assert.equal(days[0].continuation, false);
assert.ok(days.slice(1).every(day => day.continuation));
assert.ok(days.every(day => day.entries.length <= 10));
assert.ok(weeks.every(week => week.length === 7), 'weekday alignment is preserved on continuation rows');
...
console.log('Combined calendar: multi-protocol selection, compact grouping, detail option and crowded-day continuation passed');
